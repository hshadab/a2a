#!/usr/bin/env python3
"""
Generate a simple MLP ONNX model with 6 layers using MatMul + Square operations.
The model architecture:
  Input: [batch_size, 4]
  Layer 1: MatMul [4, 16] + Square
  Layer 2: MatMul [16, 16] + Square
  Layer 3: MatMul [16, 16] + Square
  Layer 4: MatMul [16, 8] + Square
  Layer 5: MatMul [8, 8] + Square
  Layer 6: MatMul [8, 4] + Square
  Output: [batch_size, 4]
...
class SquareMLP(nn.Module):
    def __init__(self):
        ...
        # Define 6 linear layers with power-of-two dimensions
        ...
        # Layer dimensions: 4 -> 16 -> 16 -> 16 -> 8 -> 8 -> 4
        ...
        self.fc5 = nn.Linear(8, 8, bias=False)
        self.fc6 = nn.Linear(8, 4, bias=False)
        
        # Initialize weights with Xavier/Glorot initialization, scaled down
        # Scale factor of 0.1 to keep values small for fixed-point arithmetic
        scale_factor = 0.1
        with torch.no_grad():
            ...
            self.fc5.weight.data *= scale_factor
            
            nn.init.xavier_uniform_(self.fc6.weight)
            self.fc6.weight.data *= scale_factor
    ...
    def forward(self, x):
        ...
        # Layer 5: MatMul + Square
        x = self.fc5(x)
        x = torch.pow(x, 2.0)
        
        # Layer 6: MatMul + Square
        x = self.fc6(x)
        x = torch.pow(x, 2.0)
...
def main():
    ...
    print("Layer 2: MatMul [16, 16] + Square -> [batch_size, 16]")
    print("Layer 3: MatMul [16, 16] + Square -> [batch_size, 16]")
    print("Layer 4: MatMul [16, 8] + Square -> [batch_size, 8]")
    print("Layer 5: MatMul [8, 8] + Square -> [batch_size, 8]")
    print("Layer 6: MatMul [8, 4] + Square -> [batch_size, 4]")
    ...
    print(f"✓ 6 layers: MatMul + Square activation")
